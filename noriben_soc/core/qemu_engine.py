...
async def run_dynamic_analysis(sample: Path, timeout: int = 300) -> dict:
    ...
    cmd = ["qemu-system-x86_64","-machine","q35","-cpu","host",
           "-smp","4","-m","4096",
           f"-drive","file={VM_IMG},if=virtio,snapshot=on",
    ...
    await _inject(sample.name, timeout)
...
async def _inject(filename: str, timeout: int):
    bat = SHARED / "run.bat"
    bat.write_text(
        "@echo off\ncd C:\\noriben\n"
        f"python noriben.py -t {timeout} "
        f"--output C:\\shared\\results\\{Path(filename).stem}.pml "
        f"--cmd C:\\shared\\samples\\{filename}\n")